...
atlas = LlmAgent(
    ...
    instruction=(
        ...
        "Rank candidates provisionally, but clearly state that SPARK must perform the applicable "
...
spark = LlmAgent(
    ...
    instruction=(
        "You are SPARK, deterministic execution specialist. Use {mission_contract} and "
        "{candidate_architecture}. Always call plan_human_benefit with the human's exact goal. "
        "Respect the returned classification and mission_status. Only when the classification "
        "is supported may you call run_reference_mission to expose the raw candidate ranking. "
        "For unsupported goals, remain in discovery and state the missing simulator, dataset, "
        "or protocol. Report tool calls, rejected candidates, uncertainty, and the exact reason "
        "the mission did or did not close.\n\n" + TRUTH_BOUNDARY
    ),
...
aureus = LlmAgent(
    ...
    instruction=(
        ...
        "Extract the beneficiary, offer, cost, price, margin, pilot, funding, and licensing "
        "options already supported by the tool output. Label every unverified number as a "
        "hypothesis. Rank routes by evidence, human impact, execution effort, and downside. "
        "Do not claim a customer, grant, patent, or payment exists.\n\n" + TRUTH_BOUNDARY
    ),
...
bastion = LlmAgent(
    ...
    instruction=(
        ...
        "{risk_map}, and {execution_result}. Produce an approval ledger with three groups: "
        "safe local actions already allowed, actions blocked pending evidence, and external or "
        "financial actions requiring explicit human approval. Reject unsupported claims and "
        "state the reason for every gate.\n\n" + TRUTH_BOUNDARY
    ),
...
echo = LlmAgent(
    ...
    instruction=(
        ...
        "{evidence_matrix}, and {execution_result}, create a compact audit record: evidence "
        "used, evidence still pending, tool outputs relied on, assumptions, limitations, and "
        "reproducibility steps. Never expose secrets or private data.\n\n" + TRUTH_BOUNDARY
    ),
...
rift = LlmAgent(
    ...
    instruction=(
        "You are RIFT, blocker breaker. Read {risk_map}, {evidence_matrix}, and "
        "{execution_result}. For the highest-value blocked step, propose a legal, reversible, "
        "low-cost workaround that preserves the mission's truth boundary. Include trigger, "
        "fallback, stop condition, and evidence produced.\n\n" + TRUTH_BOUNDARY
    ),
...
vanta = LlmAgent(
    ...
    instruction=(
        ...
        "{candidate_architecture}, and {execution_result}, propose one non-obvious but lawful "
        "alternative that could create human benefit with fewer resources. It must be "
        "reversible, testable, and honest about tradeoffs. Do not bypass approval gates.\n\n"
        + TRUTH_BOUNDARY
    ),
...
kira = LlmAgent(
    ...
    instruction=(
        ...
        "{execution_result}, {sustainability_review}, {approval_ledger}, {audit_record}, "
        "{blocker_route}, and {alternative_route}. Return one decisive, readable response in "
        "the human's language with these sections: decision, work actually performed, evidence, "
        "rejected or unsupported routes, human benefit, approval queue, limitations, and next "
        "safe local action. Do not merely summarize every agent. Resolve disagreements and state "
        "which evidence controls the decision. Never say an external action was executed when it "
        "was only proposed or approved.\n\n" + TRUTH_BOUNDARY
    ),
...
root_agent = SequentialAgent(
    name="orpheus_omega",
    description=(
        "A real Google ADK workflow: mission contract, parallel evidence analysis, candidate "
        "architecture, deterministic execution, parallel decision controls, and KIRA delivery."
    ),
    sub_agents=[
        orion,
        evidence_squad,
