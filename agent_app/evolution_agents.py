...
def build_evolution_squad(model_factory: ModelFactory, truth_boundary: str) -> ParallelAgent:
    """Create the post-execution cell that tries to falsify success before KIRA can close."""
    ...
    helix = LlmAgent(
        ...
        instruction=(
            ...
            "readiness. Use {execution_result}, {forja_test_gate}, and {recursion_audit}. Do not "
            "award points for planned work, agent names, interface copy, or unsupported claims. "
            "For every score identify the controlling evidence, missing proof, and one next test "
            "that would raise it. Also state whether mandatory submission viability is PASS or FAIL. "
            "The official per-criterion maximum is 5; bonus contributions are tracked separately.\n\n"
            + truth_boundary
        ),
    ...
    return ParallelAgent(
        name="evolution_squad",
        description="Runs evolutionary audit, ruthless lawful falsification, and evidence-only scoring concurrently.",
        sub_agents=[recursor, nemesis, helix],
    )
